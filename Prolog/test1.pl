...
tio(X, Y) :- hermanos(X,C), hijo(Y,C), hombre(X).
tia(X, Y) :- hermanos(X,C), hijo(Y,C), mujer(X).

sobrino(X, Y) :- hermanos(Y,W), hijo(X,W).
primos(X, Y) :- sobrino(X,Z), hijo(Y,Z).
bisabuelo(BAP, X) :- padre(BAP,Y), hijo(X,Z), hijo(Z,Y).
bisabuela(BAP, X) :- madre(BAP,Y), hijo(X,Z), hijo(Z,Y).
